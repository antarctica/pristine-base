...
# This resource implicitly depends on the 'aws_route53_record.£PROJECT-LOWER-CASE-prod-node1-ext' resource
resource "null_resource" "ansible-galaxy" {
  triggers {
    £PROJECT-LOWER-CASE_prod_node1_instance_id = "${aws_instance.£PROJECT-LOWER-CASE-prod-node1.id}"
    £PROJECT-LOWER-CASE_prod_node1_dns_fqdn = "${aws_route53_record.£PROJECT-LOWER-CASE-prod-node1-ext.fqdn}"
  }
